...
struct str_map_t esim_finish_map = { 21, { { "ContextsFinished", esim_finish_ctx },

{ "x86LastInst", esim_finish_x86_last_inst }, { "x86MaxInst", esim_finish_x86_max_inst }, {
		"x86MaxCycles", esim_finish_x86_max_cycles },

{ "ArmMaxInst", esim_finish_arm_max_inst }, { "ArmMaxCycles", esim_finish_arm_max_cycles },

{ "MipsMaxInst", esim_finish_mips_max_inst }, { "MipsMaxCycles", esim_finish_mips_max_cycles },

{ "EvergreenMaxInst", esim_finish_evg_max_inst },
		{ "EvergreenMaxCycles", esim_finish_evg_max_cycles }, { "EvergreenMaxKernels",
				esim_finish_evg_max_kernels }, { "EvergreenNoFaults", esim_finish_evg_no_faults }, /* GPU-REL */

		{ "FermiMaxInst", esim_finish_frm_max_inst },
		{ "FermiMaxCycles", esim_finish_frm_max_cycles }, { "FermiMaxFunctions",
				esim_finish_frm_max_functions },

		{ "SouthernIslandsMaxInst", esim_finish_si_max_inst }, { "SouthernIslandsMaxCycles",
				esim_finish_si_max_cycles }, { "SouthernIslandsMaxKernels",
				esim_finish_si_max_kernels },

		{ "MaxTime", esim_finish_max_time }, { "Signal", esim_finish_signal }, { "Stall",
				esim_finish_stall } } };
...
long long esim_domain_cycle(int domain_index) {
	...
	if (!domain)
		panic("%s: invalid domain index (%d)", __FUNCTION__, domain_index);
...
struct esim_event_info_t *esim_event_info_create(int id, char *name, esim_event_handler_t handler,
		struct esim_domain_t *domain) {
	...
	while (1) {
		...
		if (count == ESIM_MAX_FINALIZATION_EVENTS) {
			...
			warning("%s: number of finalization events exceeds %d - stopped.\n%s", __FUNCTION__,
					ESIM_MAX_FINALIZATION_EVENTS, esim_err_finalization);
			return 1;
		}
	}
...
void esim_dump(FILE *f, int max) {
	...
	fprintf(f, "Total: %d event(s)\n", esim_event_heap->count + aux_event_heap->count);
	fprintf(f, "\n");
...
int esim_register_event_with_name(esim_event_handler_t handler, int domain_index, char *name) {
	struct esim_event_info_t *event_info;
	struct esim_domain_t *domain;
	int index;
...
void esim_schedule_event(int event_index, void *data, int cycles) {
	...
	if (!esim_overload_shown && esim_event_heap->count >= ESIM_OVERLOAD_EVENTS) {
		...
		warning("%s: number of in-flight events exceeds %d.\n%s", __FUNCTION__,
				ESIM_OVERLOAD_EVENTS, esim_err_overload);
	}
}
...
void esim_process_events(int forward) {
	...
	while (1) {
		...
		struct mod_stack_t *stack;
		stack = event->data;
		if (event->id == EV_MOD_NMOESI_LOAD || event->id == EV_MOD_NMOESI_LOAD_LOCK
				|| event->id == EV_MOD_NMOESI_LOAD_UNLOCK || event->id == EV_MOD_NMOESI_LOAD_FINISH
				|| event->id == EV_MOD_NMOESI_STORE || event->id == EV_MOD_NMOESI_STORE_LOCK
				|| event->id == EV_MOD_NMOESI_STORE_UNLOCK
				|| event->id == EV_MOD_NMOESI_STORE_FINISH) {
			if (stack->addr >= 0x1f48 && stack->addr <= 0x1f54)
				fprintf(stderr, "  event %x,%lld,%lld,%d \n", stack->addr, esim_time, when,
						event->id);
		}
		event_info->handler(event->id, event->data);
		esim_event_free(event);
