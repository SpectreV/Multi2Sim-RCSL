...
void X86EmuProcessEvents(X86Emu *self) {
	...
	for (ctx = self->suspended_list_head; ctx; ctx = next) {
		...
		if (X86ContextGetState(ctx, X86ContextPoll)) {
			...
			host_fds.events = ((ctx->wakeup_events & 4) ? POLLOUT : 0)
					| ((ctx->wakeup_events & 1) ? POLLIN : 0);
	...
	for (ctx = self->context_list_head; ctx; ctx = ctx->context_list_next) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (!ctx->host_thread_timer_wakeup
					|| ctx->itimer_value[i] < ctx->host_thread_timer_wakeup)
...
void X86EmuLoadContextsFromConfig(X86Emu *self, struct config_t *config, char *section) {
	...
	int kernel_id;
	FPGAEmu *fpga_emu;
	FPGAKernel *exist_kernel, *new_kernel;
	...
	if (!*exe)
		fatal("%s: [%s]: invalid executable", config_file_name, section);
	...
	/* Added by Feng, Modified by Hao
	 * Read IDs into context kernel ID list from context configuration file
	 * */
	...
	if (token) {
		while (1) {
			...
			for (i = 0; i < list_count(ctx->kernel_list); i++) {
				exist_kernel = list_get(ctx->kernel_list, i);
				if (exist_kernel == new_kernel)
					fatal("Duplicate kernel id %d referred by the same context %d!", kernel_id,
							ctx->pid - 100);
			}
			list_add(ctx->kernel_list, new_kernel);
			mem_map(ctx->mem, new_kernel->HW_bounds.low,
					new_kernel->HW_bounds.high - new_kernel->HW_bounds.low,
