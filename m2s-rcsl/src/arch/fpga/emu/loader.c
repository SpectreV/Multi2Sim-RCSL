...
static struct str_map_t elf_section_flags_map = { 3, { { "SHF_WRITE", 1 }, {
...
void FPGAKernelAddImpsString(FPGAKernel *self, char *imps, implement_param type) {
	...
	/* Tokens */
	int i = 0;
	for (arg = strtok(imps, delim); arg; arg = strtok(NULL, delim)) {
		arg = str_set(NULL, arg);
		switch(type) {
		case WIDTH:
			linked_list_add(loader->widths, arg);
			break;
		case LENGTH:
			linked_list_add(loader->lengths, arg);
			break;
		case HEIGHT:
			linked_list_add(loader->heights, arg);
			break;
		default:
...
#define FPGA_LOADER_STACK_BASE  0xc0000000
#define FPGA_LOADER_MAX_ENVIRON  0x10000  /* 16KB for environment */
#define FPGA_LOADER_STACK_SIZE  0x800000  /* 8MB stack size */
...
void FPGAKernelLoadELFSections(FPGAKernel *self, struct elf_file_t *elf_file) {
	...
	for (i = 0; i < list_count(elf_file->section_list); i++) {
		...
		fpga_loader_debug("  section %d: name='%s', offset=0x%x, "
				"addr=0x%x, size=%u, flags=%s\n", i, section->name,
				section->header->sh_offset, section->header->sh_addr,
		...
		if (section->header->sh_flags & SHF_ALLOC) {
			...
			/* Load section */
			mem_map(mem, section->header->sh_addr, section->header->sh_size,
					perm);
			mem->heap_break = MAX(mem->heap_break,
			...
			if (section->header->sh_type == 8) {
				...
				mem_access(mem, section->header->sh_addr,
						section->header->sh_size, ptr, mem_access_init);
				free(ptr);
			} else {
				mem_access(mem, section->header->sh_addr,
						section->header->sh_size, section->buffer.ptr,
						mem_access_init);
			}
		}
	}
...
void FPGAKernelLoadProgramHeaders(FPGAKernel *self) {
	...
	fpga_loader_debug("  virtual address for program header table: 0x%x\n",
			phdt_base);
	...
	for (i = 0; i < list_count(elf_file->program_header_list); i++) {
		...
		/* Debug */
		str_map_value_buf(&elf_program_header_type_map,
				program_header->header->p_type, str, sizeof(str));
		fpga_loader_debug("  header loaded at 0x%x\n",
				phdt_base + i * phdr_size);
		fpga_loader_debug("    type=%s, offset=0x%x, vaddr=0x%x, paddr=0x%x\n",
				str, program_header->header->p_offset,
				program_header->header->p_vaddr,
				program_header->header->p_paddr);
		fpga_loader_debug("    filesz=%d, memsz=%d, flags=%d, align=%d\n",
		...
		if (program_header->header->p_type == 3) {
			mem_read_string(mem, program_header->header->p_vaddr, sizeof(str),
					str);
...
#define FPGA_LOADER_AV_ENTRY(t, v) \
{ \
...
unsigned int FPGAKernelLoadAV(FPGAKernel *self, unsigned int where) {
	...
	fpga_loader_debug("Loading auxiliary vector at 0x%x\n", where);

	/* Program headers */
	FPGA_LOADER_AV_ENTRY(3, loader->phdt_base); /* AT_PHDR */
	FPGA_LOADER_AV_ENTRY(4, 32); /* AT_PHENT -> program header size of 32 bytes */
	FPGA_LOADER_AV_ENTRY(5, loader->phdr_count); /* AT_PHNUM */

	/* Other values */
	FPGA_LOADER_AV_ENTRY(6, MEM_PAGE_SIZE); /* AT_PAGESZ */
	FPGA_LOADER_AV_ENTRY(7, 0); /* AT_BASE */
	FPGA_LOADER_AV_ENTRY(8, 0); /* AT_FLAGS */
	FPGA_LOADER_AV_ENTRY(9, loader->prog_entry); /* AT_ENTRY */
	FPGA_LOADER_AV_ENTRY(11, getuid()); /* AT_UID */
	FPGA_LOADER_AV_ENTRY(12, geteuid()); /* AT_EUID */
	FPGA_LOADER_AV_ENTRY(13, getgid()); /* AT_GID */
	FPGA_LOADER_AV_ENTRY(14, getegid()); /* AT_EGID */
	FPGA_LOADER_AV_ENTRY(17, 0x64); /* AT_CLKTCK */
	FPGA_LOADER_AV_ENTRY(23, 0); /* AT_SECURE */

	/* Random bytes */
	loader->at_random_addr_holder = sp + 4;
	FPGA_LOADER_AV_ENTRY(25, 0); /* AT_RANDOM */

	/*FPGA_LOADER_AV_ENTRY(32, 0xffffe400);
	 FPGA_LOADER_AV_ENTRY(33, 0xffffe000);
	 FPGA_LOADER_AV_ENTRY(16, 0xbfebfbff);*/

	/* ??? AT_HWCAP, AT_PLATFORM, 32 and 33 ???*/

	/* Finally, AT_NULL, and return size */
	FPGA_LOADER_AV_ENTRY(0, 0);
	return sp - where;
}
#undef FPGA_LOADER_AV_ENTRY
...
void FPGAKernelLoadStack(FPGAKernel *self) {
	...
	/* Allocate stack */
	loader->stack_base = FPGA_LOADER_STACK_BASE;
	loader->stack_size = FPGA_LOADER_STACK_SIZE;
	loader->stack_top = FPGA_LOADER_STACK_BASE - FPGA_LOADER_STACK_SIZE;
	mem_map(mem, loader->stack_top, loader->stack_size,
			mem_access_read | mem_access_write);
	fpga_loader_debug("mapping region for stack from 0x%x to 0x%x\n",
			loader->stack_top, loader->stack_base - 1);

	/* Load arguments and environment variables */
	loader->environ_base = FPGA_LOADER_STACK_BASE - FPGA_LOADER_MAX_ENVIRON;
	sp = loader->environ_base;
	argc = linked_list_count(loader->args);
	fpga_loader_debug("  saved 'argc=%d' at 0x%x\n", argc, sp);
	...
	for (i = 0; i < argc; i++) {
		...
		fpga_loader_debug("  argument %d at 0x%x: '%s'\n", i, sp, str);
		sp += strlen(str) + 1;
	}
	...
	for (i = 0; i < linked_list_count(loader->env); i++) {
		...
		mem_write_string(mem, sp, str);
		fpga_loader_debug("  env var %d at 0x%x: '%s'\n", i, sp, str);
		sp += strlen(str) + 1;
	}
...
void FPGAKernelLoadExe(FPGAKernel *self, char *exe) {
	struct fpga_loader_t *loader = self->loader;
	struct mem_t *mem = self->mem;
	struct fpga_file_desc_table_t *fdt = self->file_desc_table;
	...
	/* Alternative stdin */
	FPGAKernelGetFullPath(self, loader->stdin_file, stdin_file_full_path,
			MAX_STRING_SIZE);
	if (*stdin_file_full_path) {
		struct fpga_file_desc_t *fd;
		fd = fpga_file_desc_table_entry_get(fdt, 0);
		assert(fd);
		fd->host_fd = open(stdin_file_full_path, O_RDONLY);
		if (fd->host_fd < 0)
			fatal("%s: cannot open stdin", loader->stdin_file);
		fpga_loader_debug("%s: stdin redirected\n", stdin_file_full_path);
	}

	/* Alternative stdout/stderr */
	FPGAKernelGetFullPath(self, loader->stdout_file, stdout_file_full_path,
			MAX_STRING_SIZE);
	if (*stdout_file_full_path) {
		struct fpga_file_desc_t *fd1, *fd2;
		fd1 = fpga_file_desc_table_entry_get(fdt, 1);
		fd2 = fpga_file_desc_table_entry_get(fdt, 2);
		assert(fd1 && fd2);
		...
		fpga_loader_debug("%s: stdout redirected\n", stdout_file_full_path);
	}

	/* Load program into memory */
	FPGAKernelGetFullPath(self, exe, exe_full_path, MAX_STRING_SIZE);
	...
	FPGAKernelLoadELFSections(self, loader->elf_file);
	...
	FPGAKernelLoadProgramHeaders(self);
	if (loader->interp)
		FPGAKernelLoadInterp(self);

	/* Stack */
	FPGAKernelLoadStack(self);

	/* Register initialization */
	self->regs->eip =
			loader->interp ? loader->interp_prog_entry : loader->prog_entry;
	self->regs->esp = loader->environ_base;

	fpga_loader_debug("Program entry is 0x%x\n", self->regs->eip);
	fpga_loader_debug("Initial stack pointer is 0x%x\n", self->regs->esp);
	fpga_loader_debug("Heap start set to 0x%x\n", mem->heap_break);
}
