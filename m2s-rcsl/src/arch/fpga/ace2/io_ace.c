...
void ace_io_print_activity(Abc_Ntk_t * ntk, FILE * fp) {
	...
	for (j = 0; j < num_vec; ++j) {
		Abc_NtkForEachPo(ntk, obj, i)
		{
			info = Ace_ObjInfo(obj);
			if (fp) {
				fprintf(fp, "%d", info->values[j]);
			} else
				printf("%d", info->values[j]);
		}
		if (fp) {
			fprintf(fp, "\n");
		} else
...
int ace_io_read_activity(Abc_Ntk_t * ntk, FILE * in_file_desc,
	...
	int clk_pi_obj_id = Nm_ManFindIdByName(name_manager, (char *) clk_name,
			ABC_OBJ_PI);
	printf("Clk PI ID: %d\n", clk_pi_obj_id);
	...
	if (in_file_desc == NULL) {
		...
		if (pi_format == ACE_VEC) {
			...
			Abc_NtkForEachPo(ntk, obj_ptr, i)
			{
				info = Ace_ObjInfo(obj_ptr);
				info->values = (int *) malloc(num_vec * sizeof(int));
			}
			...
			while (!feof(in_file_desc)) {
				...
				if (strlen(vector) != num_Pi && clk_pi_obj_id == -1) {
					printf(
							"Error: vector length (%d) doesn't match number of inputs (%d)\n",
							strlen(vector), num_Pi);
					error = ACE_ERROR;
					break;
				} else if (clk_pi_obj_id != -1 && strlen(vector) == num_Pi) {
					printf(
							"Error: Please remove assignment of clock signal in vector inputs\n");
					error = ACE_ERROR;

				} else if (clk_pi_obj_id != -1
						&& strlen(vector) != num_Pi - 1) {
					printf(
							"Error: vector length (%d) doesn't match number of inputs (%d)\n",
							strlen(vector), num_Pi - 1);
					error = ACE_ERROR;
				} else {
					assert(strlen(vector) == num_Pi - 1);
				}

				if (num_vec == 0) {
					Abc_NtkForEachPi(ntk, obj_ptr, i)
					{
						if (Abc_ObjId(obj_ptr) != clk_pi_obj_id) {
							high[i] = (vector[i] == '1');
							toggles[i] = 0;
			...
			if (!error) {
				Abc_NtkForEachPi(ntk, obj_ptr, i)
				{
					if (Abc_ObjId(obj_ptr) != clk_pi_obj_id) {
						...
						info->switch_prob = (double) toggles[i]
								/ (double) num_vec;
