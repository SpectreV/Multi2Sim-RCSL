...
static enum arch_sim_kind_t fpga_sim_kind = arch_sim_kind_detailed;
...
static void m2s_load_programs(int argc, char **argv)
{
    ...
    x86_emu->kernel_list=list_create();
    fpga_emu->kernel_list=list_create();
    ...
    for (id=0;;id++)
    {
      ...
      kernel = xcalloc(1, sizeof(struct kernel_t));
	  kernel->name = xstrdup(kernel_name);
	  kernel->id = id;
	  ...
	  list_add(fpga_emu->kernel_list, kernel);
...
int main(int argc, char **argv)
{
	...
	arch_set_emu(arch_fpga, asEmu(fpga_emu));
